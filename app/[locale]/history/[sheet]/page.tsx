...
import { BAN_SHEET } from '@/lib/sheets.config';
import { notFound } from 'next/navigation';
import Tracker from '@/components/Tracker';
import LanguageSwitcher from '@/components/LanguageSwitcher';
import Link from 'next/link';

export default async function HistorySheetPage({
  params,
}: {
  params: { locale: string; sheet: string };
}) {
  ...
  return (
    <main style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', position: 'relative', zIndex: 1 }}>
      ...
      {/* Tracker in history mode — no auto-refresh, uses the archived sheet */}
      <Tracker
        t={t}
        locale={locale}
        mainSheet={sheetName}
        banSheet={BAN_SHEET}
        historySheet={sheetName}
      />
